...
BOOL QZoomView::ZoomIn(const LPPOINT pPoint /* = NULL */)
{
	...
	for (int i = 0; i < m_PresetCount; i++) if (m_Presets[i] > m_Zoom) break;
	if (i < m_PresetCount)
	{
...
BOOL QZoomView::ZoomOut(const LPPOINT pPoint /* = NULL */)
{
	...
	for (int i = m_PresetCount; --i >= 0;) if (m_Presets[i] < m_Zoom) break;
	if (i >= 0)
	{
		ZoomToPreset(i, pPoint);
		return TRUE;
...
// QZoomView message handlers

void QZoomView::OnLButtonDown(UINT nFlags, CPoint point)
{
	int mode = m_Mode;
...
BOOL QZoomView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
